...
ObstacleAvoider1::ObstacleAvoider1(const std::string& cmd_vel_topic,
  ...
  timer_robot_turn_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider1::turn180, this));

  timer_distance_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider1::updateDistance, this));

  start_time_ = this->now();

}

const char* ObstacleAvoider1::stateToString(State state) {
    switch (state) {
...
void ObstacleAvoider1::updateDistance() {
...
void ObstacleAvoider1::updateRobotBehavior() {
