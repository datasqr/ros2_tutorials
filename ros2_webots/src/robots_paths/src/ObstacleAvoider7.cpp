...
// ObstacleAvoider7 stops for 2 sec when apporach obstacle

ObstacleAvoider7::ObstacleAvoider7(const std::string& cmd_vel_topic,
                                  const std::string& left_sensor_topic,
                                  const std::string& right_sensor_topic) : Node("obstacle_avoider7"),
  ...
  left_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      left_sensor_topic, 1,
      std::bind(&ObstacleAvoider7::leftSensorCallback, this,
                std::placeholders::_1));

  right_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      right_sensor_topic, 1,
      std::bind(&ObstacleAvoider7::rightSensorCallback, this,
                std::placeholders::_1));

  timer_robot_turn_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider7::turn180, this));

  timer_distance_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider7::updateDistance, this));

  start_time_ = this->now();

}

const char* ObstacleAvoider7::stateToString(State state) {
    switch (state) {
...
void ObstacleAvoider7::leftSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  left_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider7::rightSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  right_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider7::updateDistance() {
...
void ObstacleAvoider7::updateRobotBehavior() {
...
int main(int argc, char *argv[])
{
  ...
  auto avoider = std::make_shared<ObstacleAvoider7>("/cmd_vel7", "/my_robot7/left_sensor", "/my_robot7/right_sensor");
