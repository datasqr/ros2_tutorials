...
// ObstacleAvoider4 stops for 2 sec when apporach obstacle

ObstacleAvoider4::ObstacleAvoider4(const std::string& cmd_vel_topic,
                                  const std::string& left_sensor_topic,
                                  const std::string& right_sensor_topic) : Node("obstacle_avoider4"),
  ...
  left_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      left_sensor_topic, 1,
      std::bind(&ObstacleAvoider4::leftSensorCallback, this,
                std::placeholders::_1));

  right_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      right_sensor_topic, 1,
      std::bind(&ObstacleAvoider4::rightSensorCallback, this,
                std::placeholders::_1));

  timer_robot_turn_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider4::turn180, this));

  timer_distance_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider4::updateDistance, this));

  start_time_ = this->now();

}

const char* ObstacleAvoider4::stateToString(State state) {
    switch (state) {
...
void ObstacleAvoider4::leftSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  left_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider4::rightSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  right_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider4::updateDistance() {
...
void ObstacleAvoider4::updateRobotBehavior() {
...
int main(int argc, char *argv[])
{
  ...
  auto avoider = std::make_shared<ObstacleAvoider4>("/cmd_vel4", "/my_robot4/left_sensor", "/my_robot4/right_sensor");
