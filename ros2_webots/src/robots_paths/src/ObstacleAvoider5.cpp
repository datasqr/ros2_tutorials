...
// ObstacleAvoider5 stops for 2 sec when apporach obstacle

ObstacleAvoider5::ObstacleAvoider5(const std::string& cmd_vel_topic,
                                  const std::string& left_sensor_topic,
                                  const std::string& right_sensor_topic) : Node("obstacle_avoider5"),
  ...
  left_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      left_sensor_topic, 1,
      std::bind(&ObstacleAvoider5::leftSensorCallback, this,
                std::placeholders::_1));

  right_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      right_sensor_topic, 1,
      std::bind(&ObstacleAvoider5::rightSensorCallback, this,
                std::placeholders::_1));

  timer_robot_turn_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider5::turn180, this));

  timer_distance_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider5::updateDistance, this));

  start_time_ = this->now();

}

const char* ObstacleAvoider5::stateToString(State state) {
    switch (state) {
...
void ObstacleAvoider5::leftSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  left_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider5::rightSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  right_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider5::updateDistance() {
...
void ObstacleAvoider5::updateRobotBehavior() {
...
int main(int argc, char *argv[])
{
  ...
  auto avoider = std::make_shared<ObstacleAvoider5>("/cmd_vel5", "/my_robot5/left_sensor", "/my_robot5/right_sensor");
