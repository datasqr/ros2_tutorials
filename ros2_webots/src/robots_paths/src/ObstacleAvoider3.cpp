...
// ObstacleAvoider3 stops for 2 sec when apporach obstacle

ObstacleAvoider3::ObstacleAvoider3(const std::string& cmd_vel_topic,
                                  const std::string& left_sensor_topic,
                                  const std::string& right_sensor_topic) : Node("obstacle_avoider3"),
  ...
  left_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      left_sensor_topic, 1,
      std::bind(&ObstacleAvoider3::leftSensorCallback, this,
                std::placeholders::_1));

  right_sensor_sub_ = create_subscription<sensor_msgs::msg::Range>(
      right_sensor_topic, 1,
      std::bind(&ObstacleAvoider3::rightSensorCallback, this,
                std::placeholders::_1));

  timer_robot_turn_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider3::turn180, this));

  timer_distance_ = this->create_wall_timer(100ms, std::bind(&ObstacleAvoider3::updateDistance, this));

  start_time_ = this->now();

}

const char* ObstacleAvoider3::stateToString(State state) {
    switch (state) {
...
void ObstacleAvoider3::leftSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  left_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider3::rightSensorCallback(
    const sensor_msgs::msg::Range::SharedPtr msg) {
  right_sensor_value = msg->range;
  updateRobotBehavior();
}

void ObstacleAvoider3::updateDistance() {
...
void ObstacleAvoider3::updateRobotBehavior() {
...
int main(int argc, char *argv[])
{
  ...
  auto avoider = std::make_shared<ObstacleAvoider3>("/cmd_vel3", "/my_robot3/left_sensor", "/my_robot3/right_sensor");
